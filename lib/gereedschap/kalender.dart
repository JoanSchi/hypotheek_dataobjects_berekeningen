...
class Kalender {
  ...
  static DateTime voegPeriodeToe(DateTime dateTime,
    ...
    switch (periodeOpties) {
      ...
      case PeriodeOpties.eersteDagOfVolgende:
        {
          return DateTime(huidigeJaar + jaren,
              huidigeMaand + maanden + (huidigeDag > 1 ? 1 : 0), 1);
        }
      default:
        {
          DateTime dt =
  ...
  static int deltaMaanden(DateTime startDatum, DateTime eindDatum) {
    return (eindDatum.year - startDatum.year) * 12 +
        eindDatum.month -
        startDatum.month;
  }

  static int deltaHeleMaanden(DateTime startDatum, DateTime eindDatum) {
    return deltaMaanden(startDatum, eindDatum) +
        (eindDatum.day <= startDatum.day
            ? 0
            : (eindDatum.day > startDatum.day ? 1 : 0));
  }

  static DateTime datumAlleen(DateTime datum) =>
      DateTime(datum.year, datum.month, datum.month);

  static String datumNaTekst(DateTime datum) =>
      '${datum.year}${datum.month}${datum.day}';

  static DateTime tekstNaDatum(String tekst) => DateTime.parse('yyyyMMdd');
}
